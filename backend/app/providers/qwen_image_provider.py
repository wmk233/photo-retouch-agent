...
class QwenImageProvider:
    ...
    def _parse_response(response: httpx.Response) -> dict[str, Any]:
        try:
            ...
            raise QwenProviderError(
                f"Qwen API returned invalid JSON (HTTP {response.status_code})."
            ) from exc
        ...
        if response.is_error or payload.get("code"):
            ...
            raise QwenProviderError(f"Qwen API call failed ({code}): {message}")
        return payload
    ...
    def _extract_image_url(payload: dict[str, Any]) -> str:
        try:
            ...
            raise QwenProviderError("Qwen API response did not contain an output image.") from exc

        for item in contents:
            image_url = item.get("image") if isinstance(item, dict) else None
            if image_url:
                return str(image_url)
        raise QwenProviderError("Qwen API response did not contain an output image.")

    def _save_download(self, response: httpx.Response, output_path: Path) -> None:
        if response.is_error:
            raise QwenProviderError(
                f"Failed to download Qwen output image (HTTP {response.status_code})."
            )
        content_type = response.headers.get("content-type", "")
        if content_type and not content_type.startswith("image/"):
            raise QwenProviderError("Qwen output URL did not return an image.")
        if len(response.content) > self.download_limit_bytes:
            raise QwenProviderError("Qwen output image exceeded the download limit.")
